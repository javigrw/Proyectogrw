...
public class JuegoColores {
	...
	public static int[] reiniciarStats(int[] stats) {

		stats = new int[500];
		return stats;
	}

	public static int[] iniciarStats(int[] stats) {
		int contador = 0;
		File estadisticas = new File("estadisticas");
		try {

			FileInputStream leer = new FileInputStream(estadisticas);
			while (contador < 500) {
				stats[contador] = leer.read();
				contador++;
			}
			leer.close();
			return stats;
	...
	public static void guardarStats(int[] stats) {

		File estadisticas = new File("estadisticas.txt");
		try {
			estadisticas.delete();
			estadisticas = new File("estadisticas.txt");
			estadisticas.createNewFile();
			FileOutputStream escribir = new FileOutputStream(estadisticas);
			int contador = 0;
			while (contador < size(estadisticas)) {

				escribir.write(stats[contador]);
				contador++;
			}
			escribir.close();
		} catch (IOException e) {
	...
	public static void printearpers(int[][] x, int f, int c) {
		int i, j;
		File estadisticas = new File("estadisticas.txt");
		int opcion = leerposicion(777, estadisticas);
		for (i = 0; i <= f - 1; i++) {
			System.out.println(" ");
			for (j = 0; j <= c - 1; j++) {
				if (opcion == 1) {
	...
	public static void menus(int smenu) {
		...
		case 4:
			...
			System.out.println("0.Volver al menu de MODO DE JUEGO ");
			if (leerposicion(1, estadisticas) == 0) {
				System.out.println("1.Nivel 1- Aún no hay estadisticas");
			} else {
				System.out.println("1.Nivel 1-Resuelto en " + leerposicion(1, estadisticas) + " intentos");
			}
			if (leerposicion(2, estadisticas) == 0) {
				System.out.println("2.Nivel 2- Aún no hay estadisticas");
			} else {
				System.out.println("2.Nivel 2-Resuelto en " + leerposicion(2, estadisticas) + " intentos");
			}
			if (leerposicion(3, estadisticas) == 0) {
				System.out.println("3.Nivel 3- Aún no hay estadisticas");
			} else {
				System.out.println("3.Nivel 3-Resuelto en " + leerposicion(3, estadisticas) + " intentos");
			}
			if (leerposicion(4, estadisticas) == 0) {
				System.out.println("4.Nivel 4- Aún no hay estadisticas");
			} else {
				System.out.println("4.Nivel 4-Resuelto en " + leerposicion(4, estadisticas) + " intentos");
			}
			break;
		case 5:
			clear();
			...
			break;
		case 6:
			System.out.println("EDITOR DE NIVELES");
	...
	public static void seleccionarEditor() {
		...
		case 3:
			clear();
			System.out.println("Escribe el nombre del nivel a jugar");
	...
	public static void seleccionarOpcion() {
		...
		case 1:
			clear();
			escribirposicion(1, 777, estadisticas, "estadisticas.txt");
			menus(1);
			menuPrimero();
			break;
		case 2:
			clear();
			escribirposicion(2, 777, estadisticas, "estadisticas.txt");
			menus(1);
			menuPrimero();
			break;
		case 3:
			clear();
			escribirposicion(3, 777, estadisticas, "estadisticas.txt");
			menus(1);
			menuPrimero();
			break;
		default:
	...
	public static void seleccionarProgresivo() {
		...
		case 1:
			clear();
			juego(9, 9, 2, 2, 1, null);
			break;
		case 2:
			clear();
			juego(9, 9, 3, 2, 2, null);
			break;
		case 3:
			clear();
			juego(9, 9, 3, 2, 3, null);
			break;
		case 4:
			clear();
			juego(9, 9, 3, 2, 4, null);
			break;
		default:
			break;
		}
	...
	public static void seleccionarSize(int a) {
		...
		case 1:
			clear();
			info(1, 9, 9, a, cont, 1, 0);
			menus(2);
			finJuego(9, 9, a);
			break;
		case 2:
			clear();
			info(1, 11, 11, a, cont, 1, 0);
			menus(2);
			finJuego(11, 11, a);
			break;
		case 3:
			clear();
			info(1, 15, 15, a, cont, 1, 0);
			menus(2);
			finJuego(15, 15, a);
			break;
		default:
	...
	public static void info(int caso, int f, int c, int a, int cont, int menu, int nivel) {
		...
		case 5:
			if (menu == 1) {
				if (leerposicion(((((f + c) * 4) + a) + 20), estadisticas) == 0) {
					System.out.println("No hay estadisticas aún");
					System.out.println(" ");
				} else {
					System.out.println("Menor número de pasos empleados :"
							+ leerposicion(((((f + c) * 4) + a) + 20), estadisticas));
					System.out.println(" ");
				}
				break;
			}
			if (menu == 2) {
				if (leerposicion((nivel), estadisticas) == 0) {
					System.out.print("       No hay estadisticas aún");

				} else {
					System.out.print("       Menor número de pasos empleados :" + leerposicion((nivel), estadisticas));
			...
			if (leerposicion(((((f + c) * 4) + a) + 20), estadisticas) == 0) {
				...
				System.out.print("       Menor número de pasos empleados :"
						+ leerposicion(((((f + c) * 4) + a) + 20), estadisticas));
	...
	public static void juego(int f, int c, int a, int caso, int nivel, File level) {
		...
		if (caso == 2) {
			...
			if (leerposicion(nivel, estadisticas) > cont || leerposicion(nivel, estadisticas) == 0) {
				System.out.println("FELICIDADES, HAS BATIDO EL RECORD DE ESTE TABLERO");
				System.out.println("GUARDANDO RECORD............");
				escribirposicion(cont, nivel, estadisticas, "estadisticas.txt");
			}
			clear();
			System.out.println(" ");
			menus(4);
		...
		if (caso == 1) {
			...
			if (leerposicion(((((f + c) * 4) + a) + 20), estadisticas) > cont
					|| leerposicion(((((f + c) * 4) + a) + 20), estadisticas) == 0) {
				System.out.println("FELICIDADES, HAS BATIDO EL RECORD DE ESTE TABLERO");
				System.out.println("GUARDANDO RECORD............");
				escribirposicion(cont, ((((f + c) * 4) + a) + 20), estadisticas, "estadisticas.txt");
			}
			clear();
	...
	public static void chuleta(int caso) {
		...
		switch (caso) {
		case 1:
			System.out.print("   Pulse 1 para escoger --> " + "\033[38;5;196;48;5;196m" + a + "\033[0m");
			break;
		case 2:
			System.out.print("   Pulse 2 para escoger --> " + "\033[38;5;34;48;5;34m" + a + "\033[0m");
			break;
		case 3:
			System.out.print("   Pulse 3 para escoger --> " + "\033[38;5;21;48;5;21m" + a + "\033[0m");
			break;
		case 4:
			System.out.print("   Pulse 4 para escoger --> " + "\033[38;5;226;48;5;226m" + a + "\033[0m");
			break;
		case 5:
			System.out.print("   Pulse 5 para escoger --> " + "\033[38;5;165;48;5;165m" + a + "\033[0m");
			break;
		case 6:
			System.out.print("   Pulse 6 para escoger --> " + "\033[38;5;45;48;5;45m" + a + "\033[0m");
			break;
		case 7:
			System.out.print("   Pulse 7 para escoger --> " + "\033[38;5;94;48;5;94m" + a + "\033[0m");
			break;
		case 8:
			System.out.print("   Pulse 8 para escoger --> " + "\033[37;47m" + a + "\033[0m");
			break;
		default:
			break;
		}
	...
	public static void main(String[] args) {
		int[] stats = new int[500];
		File estadisticas = new File("estadisticas.txt");
		if (leerposicion(1, estadisticas) == -1) {
			reiniciarstats();
		}
		escribirposicion(1, 777, estadisticas, "estadisticas.txt");
		menus(1);
